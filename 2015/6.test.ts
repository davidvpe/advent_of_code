const input6 = `turn on 489,959 through 759,964
...
const first = (input: string) => {
  ...
  const instructions = input.split('\n').map((inst) => {
    ...
    const [from, _, to] = instruction.split(action)[1].trim().split(' ')
  ...
  for (const instruction of instructions) {
    ...
    for (let x = instruction.fromX; x <= instruction.toX; x++) {
      for (let y = instruction.fromY; y <= instruction.toY; y++) {
        if (instruction.action === 'toggle') {
          // console.log('TURNING ', x, y, grid[x][y], '=>', !grid[x][y])
          grid[x][y] = !grid[x][y]
        } else {
          const old = grid[x][y]
          const newV = instruction.action === 'turn on'

          if (newV != old) {
            // console.log('TURNING ', x, y, grid[x][y], '=>', instruction.action === 'turn on')
          }
    ...
    for (let x = instruction.fromX; x <= instruction.toX; x++) {
      for (let y = instruction.fromY; y <= instruction.toY; y++) {
        if (instruction.action === 'toggle') {
          ...
          const newV = instruction.action === 'turn on' ? 1 : -1

          grid[x][y] += instruction.action === 'turn on' ? 1 : -1
...
describe('Excercise 6', () => {
  describe('First', () => {
    it('sample 1', () => {
      const sampleResult = first(`turn on 0,0 through 999,999`)
      expect(sampleResult).toBe(1000000)
    })

    it('sample 2', () => {
      const sampleResult = first(`toggle 0,0 through 999,0`)
      expect(sampleResult).toBe(1000)
    })

    it('sample 3', () => {
      const sampleResult = first(`turn on 499,499 through 500,500
      toggle 0,0 through 999,0
      turn off 0,0 through 0,999`)
      expect(sampleResult).toBe(1003)
    })

    it('official', () => {
      const result = first(input6)
      expect(result).toBe(569999)
    })
  })
  describe('Second', () => {
    it('sample 1', () => {
      const sampleResult = second(`turn on 0,0 through 0,0`)
      expect(sampleResult).toBe(1)
    })

    it('sample 2', () => {
      const sampleResult = second(`toggle 0,0 through 999,999`)
      expect(sampleResult).toBe(2000000)
    })

    it('official', () => {
      const result = second(input6)
      expect(result).toBe(569999)
    })
  })
})
